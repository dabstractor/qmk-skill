# 11 — Other Features: Bootmagic, DIP Switches, Unicode, Stenography, WPM, Community Modules
...
Community Modules are QMK's newer **third-party extensibility mechanism**. A module is a self-contained directory (`qmk_module.json` + `rules.mk` + `<module>.c` + optional `config.h`) that can hook into the same lifecycle/keyprocessing APIs as built-in features (`keyboard_pre_init`, `process_record`, `housekeeping_task`, suspend, shutdown, matrix/layer/RGB/LED/pointing hooks, …). Modules declare their own keycodes and feature dependencies, and are discovered via `keymap.json`. They have first-class support for **External Userspace** — strongly cross-reference `16-userspace-development.md`.

> Relationship to userspace: Community Modules are the **packaged, redistributable** counterpart to hand-rolled userspace code (`16-userspace-development.md`'s `custom_quantum_functions`). Where userspace lets *you* add `_kb`/`_user` hooks for *your* keyboards, modules let *authors* ship those hooks for *anyone's* build, including declaring new keycodes and feature deps.

### Adding a module to a build
...
### Importing a module repo into External Userspace
...
Required fields: `module_name`, `maintainer`. `license` (use an [SPDX identifier](https://spdx.org/licenses/)) and `url` are encouraged. `features` mirrors the `info.json` features block and lets the module pull in build dependencies (e.g. `deferred_exec`). `keycodes` declares new keycodes + aliases the module contributes to keymaps.
...
| `introspection.c` / `introspection.h` | yes (advanced) | Hook into keymap introspection: header is prepended before the user keymap; source appended after. Advanced — follow existing patterns. |
...
#### Compatible APIs (the hook surface)
...
- `introspection.c/.h` is powerful but **advanced and easy to get wrong** — follow existing module patterns; ask on Discord/issues if unsure.
- `features` in `qmk_module.json` is the same schema as keyboard `info.json` features, so a module can pull in `deferred_exec`, RGB matrix, split sync, etc. — but each adds firmware size.
...
| Community Modules | (none; declared in `keymap.json`) | `process_record_<module>`, `keyboard_pre_init_<module>`, … | userspace → `16`; split sync → `10` |
